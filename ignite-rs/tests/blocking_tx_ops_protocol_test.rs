...
async fn should_pin_explicit_transaction_to_start_channel_and_commit_on_same_node() {
    ...
    // tx_start, cache_get, and tx_end must all be pinned to the same node
    let seed_tx = seed.recorded_tx_starts();
    let disc_tx = discovered.recorded_tx_starts();
    let (pinned_tx_starts, other_tx_starts, pinned_cache, other_cache, pinned_tx_ends) =
        if seed_tx.len() == 1 {
            (
                seed_tx,
                disc_tx,
                seed.recorded_cache_requests(),
                discovered.recorded_cache_requests(),
                seed.recorded_tx_ends(),
            )
        } else {
            (
                disc_tx,
                ...
                discovered.recorded_cache_requests(),
                seed.recorded_cache_requests(),
                discovered.recorded_tx_ends(),
            )
        };
    assert_eq!(pinned_tx_starts.len(), 1);
    assert_eq!(other_tx_starts.len(), 0);
    assert_eq!(pinned_tx_starts[0].timeout_ms, 777);
    assert_eq!(pinned_tx_starts[0].label.as_deref(), Some("phase3"));
    assert_eq!(pinned_cache.len(), 1);
    assert_eq!(pinned_cache[0].tx_id, Some(7));
    assert!(other_cache.is_empty());
    assert_eq!(pinned_tx_ends.len(), 1);
    assert_eq!(pinned_tx_ends[0].tx_id, 7);
    assert!(pinned_tx_ends[0].committed);
}
