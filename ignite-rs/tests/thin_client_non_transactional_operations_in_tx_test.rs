...
/// Java parity: org.apache.ignite.internal.client.thin.ThinClientNonTransactionalOperationsInTxTest#testClearKeyNotAllowedInTx
#[tokio::test]
async fn should_reject_clear_key_operations_inside_explicit_transaction() {
    let client = connect().await.unwrap();
    let cache_name = unique_name("tx_clear_key");
    destroy_cache_if_exists(&client, &cache_name).await;
    client
        .get_or_create_cache::<i32, i32>(&cache_name)
        .await
    ...
    let cache = tx.cache::<i32, i32>(&cache_name);
    let err = cache.clear_key(&1).await.unwrap_err();
    let msg = err.to_string();
...
/// FND-027: CACHE_CLEAR_KEYS inside a tx must be rejected client-side, matching
/// Java's ThinClientNonTransactionalOperationsInTxTest#testClearKeysNotAllowedInTx.
#[tokio::test]
async fn should_reject_clear_keys_operations_inside_explicit_transaction() {
    let client = connect().await.unwrap();
    let cache_name = unique_name("tx_clear_keys");
    destroy_cache_if_exists(&client, &cache_name).await;
    client
        .get_or_create_cache::<i32, i32>(&cache_name)
        .await
    ...
    let cache = tx.cache::<i32, i32>(&cache_name);
    let err = cache.clear_keys(&[1, 2]).await.unwrap_err();
    let msg = err.to_string();
