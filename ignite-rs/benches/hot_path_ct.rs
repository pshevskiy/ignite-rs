//! PFND-008 — current-thread runtime variant of `hot_path`.
//!
//! Mirrors the structure of `hot_path.rs` but drives the bench body on a
//! `new_current_thread()` tokio runtime instead of the default multi-thread
//! one. All other setup (mimalloc, criterion config, sample sizes) is
//! identical so the results are directly comparable.
//!
//! Run:
//!   IGNITE_ADDR=127.0.0.1:10800 cargo bench --bench hot_path_ct
...
fn ct_runtime() -> Runtime {
    Builder::new_current_thread()
        .enable_all()
        .build()
        .expect("current-thread tokio rt")
}
...
fn bench_put_single(c: &mut Criterion) {
    ...
    let cache = rt.block_on(make_cache(&client, "HOTPATH_CT_PUT_SINGLE"));

    let mut group = c.benchmark_group("put_single_ct");
...
fn bench_get_single(c: &mut Criterion) {
    let rt = ct_runtime();
    let client = rt.block_on(make_client());
    let cache = rt.block_on(make_cache(&client, "HOTPATH_CT_GET_SINGLE"));
    rt.block_on(seed(&cache, 1024, 16));

    let mut group = c.benchmark_group("get_single_ct");
    group.measurement_time(Duration::from_secs(5));
    group.sample_size(50);
...
fn bench_put_all(c: &mut Criterion) {
    let rt = ct_runtime();
    let client = rt.block_on(make_client());
    let cache = rt.block_on(make_cache(&client, "HOTPATH_CT_PUT_ALL"));

    let mut group = c.benchmark_group("put_all_ct");
    group.measurement_time(Duration::from_secs(5));
